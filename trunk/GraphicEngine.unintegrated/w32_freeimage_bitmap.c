  ...
  switch(imgtype)
    {
      case FIF_BMP:      // Windows or OS/2 Bitmap  
       // dib = FreeImage_LoadBMP(fname, BMP_DEFAULT); ***Original 
		  dib = FreeImage_Load(imgtype,fname,BMP_DEFAULT); //***May
        break;
      ...
      case FIF_PNG:      // Portable Network Graphics  
        //dib = FreeImage_LoadPNG(fname, PNG_DEFAULT);  ***Original
		  dib = FreeImage_Load(imgtype,fname,PNG_DEFAULT); //***May
        break;
