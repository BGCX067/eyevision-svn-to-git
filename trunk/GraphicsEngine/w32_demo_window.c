...
evsShowMouse = EVSNOMOUSE;     //esk - initialize var
...
{                     
  ...
  switch (message)
	...
	break;
      ...
      //esk - windows passes this message every time the cursor moves.
      //      The default behavior of this case was to never allow the cursor to be seen.
      //      Added outer if/else to check whether EVS wants to display cursor or not. 
	  case WM_SETCURSOR:   // Cursor moving into window area

		  if (evsShowMouse == EVSYESMOUSE) {	//esk - check if cursor is supposed to be displayed
              SetCursor(arrow_cursor);
		  } 
		  else {	//esk - otherwise, we're going to use the original code/behavior
           
              if(full_screen_window_active) 
                   SetCursor(NULL);          // If covering full screen, hide it
              else 
                   SetCursor(arrow_cursor);  // Otherwise, keep cursor visible
		  }	// esk - end outer if/else
          return TRUE;	// esk - original return statement.
